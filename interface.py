...
class UI(QTabWidget):
    ...
    def engine_bttn(self):
        df, ggr, steps = engine(float(self.lineedit1.text()), float(self.lineedit2.text()))
    ...
    def optimBistr_bttn(self):
        ...
        ratioPerStep, bistr = optim_bistr(int(self.lineedit5.text()), float(self.lineedit3.text()), int(self.lineedit4.text()))
        for i in range(int(self.lineedit4.text())):
            self.plainTextEdit.appendPlainText(f"{i+1}-я ступень U = {ratioPerStep[i]}")
    ...
    def optimMert_bttn(self):
        ...
        ratioPerStep, bistr = optim_mert(int(self.lineedit6.text()), float(self.lineedit3.text()), int(self.lineedit7.text()))
        for i in range(int(self.lineedit7.text())):
            self.plainTextEdit2.appendPlainText(f"{i+1}-я ступень U = {ratioPerStep[i]}")
